...
internal static class CommandsConfigurationExtensions
{
    internal static IResourceBuilder<T> WithLifeCycleCommands<T>(this IResourceBuilder<T> builder) where T : IResource
    {
        builder.WithCommand(
            "start",
            "Start",
            resourceSnapshot => Task.FromResult(IsStopped(resourceSnapshot.State?.Text)),
            async (resourceName, services) =>
            {
                await services.GetRequiredService<ApplicationExecutor>().StartResourceAsync(resourceName).ConfigureAwait(false);
            },
        ...
        builder.WithCommand(
            ...
            async (resourceName, services) =>
            {
                await services.GetRequiredService<ApplicationExecutor>().StopResourceAsync(resourceName).ConfigureAwait(false);
            },
